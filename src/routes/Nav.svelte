...
<div class="navbar">
	<a href="/" class="logo"><h1>Fairgrounds Pizza</h1></a>
		...
		<ul>
			<li on:click={() => (isMenuOpen = false)} on:keydown={() => (isMenuOpen = false)}>
				<a
					...
					on:click={() => (current = 0)}
					href="https://www.youtube.com/watch?v=dQw4w9WgXcQ/">CALL NOW</a
				>
			</li>
			<li on:click={() => (isMenuOpen = false)} on:keydown={() => (isMenuOpen = false)}>
				<a class:current={current === 1} on:click={() => (current = 1)} href="/Menu/">Menu</a>
			</li>
			<li on:click={() => (isMenuOpen = false)} on:keydown={() => (isMenuOpen = false)}>
